...
public class Test
{
    public static void Main()
    {
        ...
        var plants = FastIO.ReadASCIIString();
        var K = FastIO.ReadNonNegativeInt();

        var plantsRequired = new int[26];
        var check = new int[26];

        for (int i = 0; i < K; i++)
        {
            var line = FastIO.ReadASCIIString().Split(" ", StringSplitOptions.RemoveEmptyEntries);
            plantsRequired[line[1][0] - 'a'] = Convert.ToInt32(line[0]);
        }
        ...
        {
            check[plants[r] - 'a']++;
            if (check[plants[r] - 'a'] == plantsRequired[plants[r] - 'a'])
                count++;
            if (count == K)
            {
                while (check[plants[l] - 'a'] > plantsRequired[plants[l] - 'a'])
                {
                    l++;
                    check[plants[l - 1] - 'a']--;
                }
                maxCount = Math.Min(maxCount, r - l + 1);
            }
            r++;
        ...
        else Console.WriteLine("Andy rapopo");
    }
